...
MATRIX_ROOT="${MATRIX_ROOT:-/mnt/d/datasets/svwatergo/domain_matrix/state_mode_tw_source}"
TARGET_COL="${TARGET_COL:-state__mode_tw}"
...
row = {
    ...
    "macro_f1": tm.get("macro_f1", tm.get("f1_macro")),
...
for TRAIN_SPEC in "${DOMAIN_SPECS[@]}"; do
  ...
  if (( TRAIN_ROWS < MIN_TRAIN_ROWS || TEST_ROWS < MIN_TEST_ROWS )); then
    ...
    for EVAL_SPEC in "${DOMAIN_SPECS[@]}"; do
      IFS='|' read -r EVAL_SITE EVAL_SOURCE _ _ _ _ _ _ <<< "$EVAL_SPEC"
