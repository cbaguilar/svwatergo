...
PLOT_META="${PLOT_META:-/mnt/d/datasets/svwatergo/derived/plots/bluerock_10s_tiny_cnn_multilabel_ro_delivery_4panel.json}"
...
"$PYTHON" -m python.ml.cli.audio_pca_svm_plot \
  ...
  --out-png "$PLOT_PNG" \
  --out-meta "$PLOT_META" \
  --title "Audio Tiny CNN Multilabel (bluerock 10s, ro+delivery)"
