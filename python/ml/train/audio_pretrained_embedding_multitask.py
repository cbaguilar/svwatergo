...
def fit_audio_pretrained_embedding_multitask(
    ...
    *,
    ...
    if mode == "multiclass":
        ...
        df0 = df.copy()
        y_meta = {"task": "plc_pca_encoder", "target_cols": []}
        y_all = np.zeros((len(df0), 1), dtype=np.float32)
    ...
    for ep in range(1, n_epochs + 1):
        ...
        if is_plc_encoder:
            ...
            print(
                f"[epoch {ep:03d}/{n_epochs}] loss={avg_loss:.6f} test_plc_r2={rtxt} lr={float(optim.param_groups[0]['lr']):.6g}",
                flush=True,
            )
        else:
    ...
    if len(idx_val):
        ...
        if mode == "multiclass":
            ...
            yva_t, yva_p = np.zeros((0,), dtype=np.float32), np.zeros((0,), dtype=np.float32)
    ...
    if Z_plc is not None and Z_plc_pred is not None:
        ...
        if int(Z_plc.shape[1]) >= 3:
            try:
                ...
                # Multicolor PLC PCA view: source/state/flow/pressure derivatives on true and predicted spaces.
                color_specs: List[Tuple[str, str, str]] = []
                color_specs.append(("source", "__source__", "categorical"))
                state_col = next(
                    (c for c in ("state__mode", "state_mode", "state__mode_tw", "primary_class", "state", "mode") if c in plc_proj_df.columns),
                    None,
                )
    ...
    if int(Z_pann.shape[1]) >= 2:
        try:
            ...
            # Additional grid with different color modes: source/state/flow/pressure derivatives.
            color_specs: List[Tuple[str, str, str]] = []
            color_specs.append(("source", "__source__", "categorical"))
            state_col = next(
                (c for c in ("state__mode", "state_mode", "state__mode_tw", "primary_class", "state", "mode") if c in pann_proj_df.columns),
                None,
            )
            if state_col is not None:
                color_specs.append((f"state__mode ({state_col})", str(state_col), "categorical"))
            ...
            if int(Z_pann.shape[1]) >= 3:
                ...
                # Shared linear axis bounds across all PANN multicolor 3D panels.
    ...
    if (not is_plc_encoder) and len(idx_test) > 0:
        src = src_series.iloc[idx_test].astype(str).fillna("unknown").to_numpy()
        for s in sorted(set(src.tolist())):
            m = src == s
            if int(np.sum(m)) <= 0:
