...
from ..train.audio_tiny_cnn import predict_audio_tiny_cnn


def main() -> int:
    p = argparse.ArgumentParser(description="Run inference with a trained audio model bundle (PCA+SVM or tiny CNN)")
    p.add_argument("--model", required=True, help="Model bundle path (.joblib for pca_svm, .pt for tiny_cnn)")
    p.add_argument("--model-kind", choices=["auto", "pca_svm", "tiny_cnn"], default="auto")
